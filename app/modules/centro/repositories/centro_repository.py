...
class CentroUniversitarioRepository:
    ...
    def get(self, centro_id: int) -> CentroUniversitario | None:
        statement = select(CentroUniversitario).where(CentroUniversitario.id == centro_id)
    ...
    def list(self, filters: dict) -> tuple[list[CentroUniversitario], int]:
        ...
        if filters.get("siiau_id") is not None:
            statement = statement.where(CentroUniversitario.siiau_id == filters["siiau_id"])
            total_statement = total_statement.where(CentroUniversitario.siiau_id == filters["siiau_id"])
